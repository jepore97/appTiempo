    ...
    for (var i=0; i < data["list"].length; i++) {
        ...
        if((dias-diaActual)==1) {
            if (data["list"][i]['main'].temp>maxd1) {
                maxd1=Math.round(data["list"][i]['main'].temp)
            }
            if (data["list"][i]['main'].temp<mind1) {
                mind1=Math.round(data["list"][i]['main'].temp)
            }
            document.getElementById("txt-temp-item1").innerHTML=maxd1+"° / "+mind1+"°"
            ...
            if (data["list"][i]['main'].temp>maxd2) {
                maxd2=Math.round(data["list"][i]['main'].temp)
            }
            if (data["list"][i]['main'].temp<mind2) {
                mind2=Math.round(data["list"][i]['main'].temp)
            }
            document.getElementById("txt-temp-item2").innerHTML=maxd2+"° / "+mind2+"°"
            ...
            if (data["list"][i]['main'].temp>maxd3) {
                maxd3=Math.round(data["list"][i]['main'].temp)
            }
            if (data["list"][i]['main'].temp<mind3) {
                mind3=Math.round(data["list"][i]['main'].temp)
            }
            document.getElementById("txt-temp-item3").innerHTML=maxd3+"° / "+mind3+"°"
